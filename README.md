...
- **Settings** — Wi-Fi, Bluetooth, SSH, Samba, Syncthing, GPU, HDMI, OTG, second SD, governors, undervolt, backup.
...
A second card in the other slot is picked up if it has a `roms/` or `Roms/` folder using the **same** `Pretty (TAG)` **names** as the table. Saves are `Saves/`. The Bios column is relative to `Bios/` on ZLYME (RetroArch `system` dir), or `Bios/` at the root of that second card.
...
| ----------------------- | -------------------------------- | ------------------------------------------------- | ------------------------------------------------------------------------- | ------------------------------------------------------- |
| NES                     | Nestopia (libretro)              | `Roms/Nintendo Entertainment System (FC)/`        | `.nes` `.unif` `.unf` `.fds` `.zip` `.7z`                                 | `disksys.rom` for FDS                                   |
...
| PC Engine               | Mednafen PCE Fast (libretro)     | `Roms/PC Engine (PCE)/`                           | `.pce` `.cue` `.ccd` `.iso` `.img` `.chd` `.sgx` `.zip` `.7z` `.m3u`      | `syscard3.pce` for CD                                   |
| SuperGrafx              | Mednafen SuperGrafx (libretro)   | `Roms/SuperGrafx (SGX)/`                          | `.sgx` `.pce` `.cue` `.ccd` `.chd` `.zip` `.7z`                           | —                                                       |
| ColecoVision            | Gearcoleco (libretro)            | `Roms/ColecoVision (COLECO)/`                     | `.col` `.bin` `.rom` `.zip` `.7z`                                         | `coleco.rom`                                            |
...
| Neo Geo (AES/MVS)       | FinalBurn Neo (libretro)         | `Roms/FBNeo (FBNEO)/`                             | `.zip` `.7z`                                                              | `neogeo.zip` in the ROM folder                          |
| Arcade                  | MAME 2003-Plus (libretro)        | `Roms/MAME (MAME)/`                               | `.zip` `.7z`                                                              | BIOS zips in the ROM folder                             |
| Neo Geo CD              | NeoCD (libretro)                 | `Roms/Neo Geo CD (NEOCD)/`                        | `.cue` `.iso` `.chd`                                                      | `neocd.bin` or `uni-bioscd.rom`                         |
...
| Atari ST                | Hatari (libretro)                | `Roms/Atari ST (ST)/`                             | `.st` `.msa` `.stx` `.dim` `.ipf` `.zip` `.7z`                            | `ST/tos.img`                                            |
...
| Pico-8                  | Official Pico-8 (add `pico8_64`) | `Roms/Pico-8 (PICO)/`                             | `.p8` `.png` `.zip`                                                       | `PICO/pico8_64` `PICO/pico8.dat` (or next to the carts) |
...
| Dreamcast               | Flycast                          | `Roms/Sega Dreamcast (DC)/`                       | `.cdi` `.gdi` `.cue` `.chd` `.m3u`                                        | `dc_boot.bin` `dc_flash.bin`                            |
