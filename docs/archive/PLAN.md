...
**Where we are (2026-09-20).** The 01:32 host image deferred udev and
dropped S11modules; the pad never loaded. Tree puts S10udevd and
S11modules back in Class A. NextUI pin is `ae652648…-zlyme40`. Do
not Etcher a games card. Do not start a second host `./build.sh` on
`output/`. GC/Wii Dolphin still dies on video backend. Wine 11.0
still cannot load PE `kernel32.dll` on first prefix.

When I say **commit**: ensure leftover attempts that should be deleted are gone. Comments in the files and the commit message stay written like a person wrote them. Split into more than one commit when the tree holds more than one reason (a hang fix is not a first-frame cut). Do not squash unrelated work into a single “boot” commit.
...
Propose wording for `README.md`, `NOTES.md`, `PLAN.md`, and `LOGBOOK.md` when it would help; do not edit those files unless I ask.
...
- **PortMaster and box64** — packaged. Image has `bash` and a real
`/roms/ports` directory (not a symlink). PORTS.pak bind-mounts the
active library's `Roms/Ports (PORTS)/` onto it. Gravity Defied
launched through PORTS.pak.
...
`nextui.elf`, `start_drastic`, and `zlyme-library`. Host already has
a 2026-09-20 `zlyme.img` (dirty tar) with NextUI rebuilt; first-flip
cuts and the `/roms/ports` mkdir fix are in that image if
target-finalize ran after those edits. Do not Etcher a games card.
Do not start a second `./build.sh`.
...
- Next Etcher of `output/images/zlyme.img` (OS card only) so PATH
copies and the 2026-09-20 init cuts are squashfs. Then read
`/tmp/boot-timing` `nextui-first-flip` vs live zlyme39 **7.6 s**.
