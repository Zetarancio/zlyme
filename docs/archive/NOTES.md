...
| **SpruceOS** | `/home/ale/SPRUCEOS/spruceOS` (home until moved to `/run/media/ale/SPCC/Cursor/MIYOO-FLIP/SPRUCEOS/spruceOS`) | `90a10ed1a` 2026-09-11 “Hold onto your butts!” (`main`) | `origin/main` **`c104be3fd54c`**; `origin/Development` **`4732b5a46a7c`** (2026-09-11 release notes). Tags `v4.4.0` / `v4.4.1`. Also `Zetarancio/spruceOS`. | Governors, pak ports, boxart (libretro-thumbnails / PyUI), HID reconnect. Prefer **Development** for “what’s new”. |
...
S10udevd and S11modules stay Class A. Dropping them for a first-flip
cut left no `retrogame_joypad` (`/dev/input` was only pwrkey, jack,
volume, hall) and NextUI went into hybrid sleep with nothing to
wake it. S26 still `modprobe`s the pad as a backup. S29udevtrigger
stays after first-flip. S17 rescans storage once more after 2 s.
Clean boot does not spawn `zlyme-update` (S18 builtin queue check).
rcS skips `S12bootfs` when `/boot` is already mounted and `S13resize`
unless `autoresize=true`. Initramfs sets `performance` before
MMC/squashfs (needs a new **Image**, not squashfs OTA alone).
`boot_mark` is a builtin read of `/proc/uptime`. Session does not
write `/storage/.../boot-timing` on the hot path (rc.late copies
it). Stock paks: trust `paks-version`; no per-pak walk before the
list.

Meter is `/tmp/boot-timing` (also copied to
`/storage/.config/zlyme/boot-timing`). Live **zlyme39** first flip
**7.6 s** (zlyme37 14.0, zlyme38 12.0). BASEOS documents rcS ~70 ms
and first NextUI frame ~5.74 s; Zlyme has not re-measured after the
2026-09-20 init cut. Initramfs keeps `ZLYME` mounted so S15 is a
no-op (~0.02 s). rc.late and Tools binds wait for
