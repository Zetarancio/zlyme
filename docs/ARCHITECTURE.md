...
- The retired ROCKNIX gamepad package is not part of the product. InputPlumber v0.81.0 starts after the first frame. `zlyme-input` then sets `ManageAllDevices` and keeps one composite per player controller. It re-enables management if InputPlumber restarts. The built-in map sends D-pad buttons to hats, digital L2/R2 (`BTN_TL2`/`BTN_TR2`) to binary `ABS_Z`/`ABS_RZ`, and the printed A/B/X/Y buttons to Xbox A/B/X/Y on the `xb360` target. NextUI uses that virtual controller after it appears. Settings → Joysticks releases only the built-in composite, uses the physical pad, then reclaims it. MENU stays on that managed gamepad (`BTN_MODE` physically, Guide on the virtual pad). Volume, power, and the lid switch remain independent system inputs;
- lid switch;
- power key;
