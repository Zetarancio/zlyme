...
## Phase 2C initramfs warning patch

The earlier note left `9998` deferred until a boot log showed whether it hid a real unpack failure. Source and config inspection settled that without a boot log. `9998` changes only the `printk` in the `#else` of `#ifdef CONFIG_BLK_DEV_RAM` in `init/initramfs.c`. Both my355 defconfigs use `board/my355/linux/linux.config` and `my355.fragment`, and that config sets `CONFIG_BLK_DEV_RAM=y`. The generated kernel config for this build also has `CONFIG_BLK_DEV_RAM=y` and `CONFIG_BLK_DEV_INITRD=y`. The `KERN_EMERG` line therefore stays in the inactive branch. It is not a live Zlyme boot-error suppression.

Zlyme's initramfs is separate. `BR2_PACKAGE_ZLYME_INITRAMFS=y`, and `board/my355/board.mk` requires an executable `images/initramfs/init` before it sets `CONFIG_INITRAMFS_SOURCE` to that directory. This build used `CONFIG_INITRAMFS_SOURCE="/zlyme/output/images/initramfs"`. The kernel linked `usr/initramfs_data.cpio`, and that archive contains `/init` starting with the Zlyme ash script.

Removed `40-kernel-7.0/linux/9998-silence-initramfs-unpack-warn.patch`. Active Linux patches: 21 before, 20 after. `linux-dirclean` then a product build extracted pristine 7.0.2 and applied 20 patches. `9998` was not among them. The build log has no "Initramfs unpacking failed" line. `post-build.sh` reported no module ABI mismatch. The Flip DTB stayed `3197ed1b9f39d368689359e8a852e3169bc09253b8c379e72344fcc3a4bf10d4`. The only kernel warnings were the existing adc-keys prototypes. No Flip test was required. OTA: `output/images/zlyme-my355-20260923-34a6114552cf-dirty.tar`, sha256 `446e7c7b7e3e5eb93d1fcc5d74743160d577d096fd5b4f70a35ec274d16652a7`. OTP `0022`/`0023` remains open. DFI and DMC were not touched.

## Phase 2C OTP candidate

Phase 2 kernel-patch work ends here. The Flip booted this candidate. DFI stays in Phase 6. DMC stays in Phase 7. Joypad architecture stays in Phase 3.

Linux 7.0.2 does not contain RK3568 OTP support. Current mainline does, with the same cell map, but a different binding: `nvmem-layout`, clock names `otp`/`apb_pclk`/`phy`/`sbpi`, and four resets. The carried patches use clock names `usr`/`sbpi`/`apb`/`phy` and one reset, `otp_phy`. This candidate does not import that newer binding.

On the running Flip (`Linux 7.0.2 #1 SMP PREEMPT Wed Sep 23 02:17:38 UTC 2026`) the provider probed with no OTP errors. It is `rockchip-otp0` at `/sys/bus/nvmem/devices/rockchip-otp0`, bound to `fe38c000.otp` / `otp@fe38c000`, compatible `rockchip,rk3568-otp`, driver `rockchip-otp`. A read returned 128 bytes, nonzero, sha256 `1101678a2d3d9ada47ed6a9f5bf37835ee5bdb7af97c912305c0dfda72fd6dd2`. The seven cells are present. The live device tree has no `nvmem-cells` property, and no Zlyme package, OPP, DMC, thermal zone, regulator, boost policy, or boot script consumes those cells.

Removed `0022` and `0023`. Active Linux patches: 20 before, 18 after. `CONFIG_NVMEM`, `CONFIG_NVMEM_SYSFS`, and `CONFIG_NVMEM_ROCKCHIP_OTP` stay `=y`. The clean rebuild has no RK3568 OTP code and no `otp@fe38c000` node. `post-build.sh` reported no module ABI mismatch. The only kernel warnings were the existing adc-keys prototypes.

Old Flip DTB `3197ed1b9f39d368689359e8a852e3169bc09253b8c379e72344fcc3a4bf10d4`. New Flip DTB `056f5f7a554a363b55ec95dd3c1d280075a9e9fd6601e5ee79ee285656bf35ce`. The decompiled difference is only the deletion of `otp@fe38c000` and its seven cells. CPU OPP, GPU, DMC, thermal, regulators, RK817, display, audio, storage, USB, and the joypad nodes are unchanged. OTA: `output/images/zlyme-my355-20260923-000c8b1ae9d1-dirty.tar`, sha256 `7bd23b856bf5d910f45f8476c1ed9e19ecb53ff17ca0c5392f74fc22b5345486`.

The installed candidate is `Linux 7.0.2 #1 SMP PREEMPT Wed Sep 23 12:49:33 UTC 2026`. `rockchip-otp0` and `otp@fe38c000` are absent. There is no OTP or NVMEM probe error. CPU cpufreq, GPU devfreq, DMC, and both thermal zones are present and readable. NextUI is up. The joypad, RK817 audio, Wi-Fi, and `/storage` are healthy. Physical checks passed for boot, LCD, backlight, controls, speaker, headphones, Wi-Fi, and standard suspend/resume. Active Linux patches: 18.
