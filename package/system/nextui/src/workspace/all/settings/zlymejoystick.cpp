...
static void screen_test(void)
{
	...
	while (g_screen) {
		...
		text_at("Final output. B back.", 24, 48);
		for (int s = 0; s < 2; s++) {
			int cx = s ? 460 : 180;
			int cy = 240;
...
static void screen_tune(bool right)
{
	...
	while (g_screen) {
		...
		snprintf(line, sizeof(line), "%d%%   Left/Right adjust   A save   B cancel", pct);
		text_at(line, 24, 48);
		int cx = 320, cy = 250;
...
static void screen_cal(bool right)
{
	...
	int step = 0;
	int stable = 0;
	char status[160] = "Rotate the stick fully around the edge. Press A when finished.";
	if (unavailable())
		return;
	cal_cap_reset(&cap);
	while (g_screen) {
		...
		text_at(status, 24, 48);
		snprintf(line, sizeof(line), "raw X=%d Y=%d  span X %d-%d  Y %d-%d",
			 x, y, cap.x_min, cap.x_max, cap.y_min, cap.y_max);
		text_at(line, 24, 88);
		if (stable)
			text_at("Center stable. A saves. B cancels.", 24, 120);
		fill_dot(g_screen, 320, 280, 70, SDL_MapRGB(g_screen->format, 40, 40, 40));
		fill_dot(g_screen, 320 + (x - 128) * 60 / 128, 280 + (y - 128) * 60 / 128, 6,
			 SDL_MapRGB(g_screen->format, 240, 240, 240));
		GFX_flip(g_screen);
		if (PAD_justPressed(BTN_B))
			break;
		if (PAD_justPressed(BTN_A)) {
			if (step == 0) {
				...
				step = 1;
				cal_center_window_reset(&cap);
				snprintf(status, sizeof(status), "Release the stick and leave it untouched.");
				continue;
			}
			if (!stable)
				continue;
			{
				...
				if (cal_cap_finish(&cap, &cfg, &err)) {
					snprintf(status, sizeof(status), "%s", err ? err : "Invalid");
					stable = 0;
					cal_center_window_reset(&cap);
					continue;
				}
...
void Zlyme_appendJoystickItem(std::vector<AbstractMenuItem *> &items)
{
	...
	std::vector<AbstractMenuItem *> sub = {
		...
		new MenuItem{ListItemType::Button, "Tune Left Deadzone", "Live preview. A saves.", go_dzl},
		new MenuItem{ListItemType::Button, "Tune Right Deadzone", "Live preview. A saves.", go_dzr},
