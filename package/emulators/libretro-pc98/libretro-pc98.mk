...
# GCC 14 treats UINT8 vs UINT IO callback pointers as an error.
define LIBRETRO_PC98_BUILD_CMDS
	$(TARGET_CONFIGURE_OPTS) \
	    CFLAGS="$(TARGET_CFLAGS) -Wno-error=incompatible-pointer-types" \
	    $(MAKE) CXX="$(TARGET_CXX)" CC="$(TARGET_CC)" -C $(@D)/sdl2/ \
	    -f Makefile.libretro platform="$(LIBRETRO_PC98_PLATFORM)" \
